...
// Segmented Iterator interface
inline True IsSegmented(HashMultiSetIterator)
{
    return True();
}

inline List::Iterator Local(HashMultiSetIterator& it)
{
    return it.pos;
}

inline List* Segment(HashMultiSetIterator& it)
{
    return it.bptr;
}

// A hash is an array of lists; hence, the segments are
// lists. We need to be able to fetch
// a local iterator over the elements within the 
// segment. We use Begin() and End() for that.
inline List::Iterator Begin(List& list)
{
    return list.Begin();
}

inline List::Iterator End(List& list)
{
    return list.End();
}
